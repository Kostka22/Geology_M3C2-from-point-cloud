...
# ── CPU thread count — set once at startup, updated by GUI ───────────
_N_THREADS = max(1, (os.cpu_count() or 4) - 1)  # leave one core for OS/GUI
...
def calculate_normals(corepoints, ref_points, k_neighbors=10, progress_mgr=None):
    ...
    _, indices = tree.query(corepoints, k=k, workers=_N_THREADS)
    if progress_mgr:
...
def generate_report_pdf(output_path, report_data, progress_mgr=None):
    ...
    sigma_mode = ('Manual' if rd.get('use_manual_sigma')
                  else 'Automatic (from py4dgeo SPREAD1/SPREAD2)')
...
def run_m3c2_pdal(las_file1, las_file2, output_file,
    ...
    normals = calculate_normals(corepoints, ref_points,
                                k_neighbors=10, progress_mgr=None)
    nz, nxy = calculate_normal_angles(normals, progress_mgr=None)
    dx, dy, dz = calculate_normal_components(distances, normals, progress_mgr=None)
    ...
    if use_pdal:
        ...
        try:
            (corepoints, distances, uncertainties,
             ref_points, epoch2_cloud, normals,
             significant, detection_limit, sigma1, sigma2,
             nz, nxy, dx, dy, dz, valid_mask) = run_m3c2_pdal(
        ...
        try:
            py4dgeo.set_num_threads(_N_THREADS)
        except Exception:
            pass
        ...
        # Step 4
        normals = calculate_normals(corepoints, ref_points,
                                    k_neighbors=10, progress_mgr=progress_mgr)
    ...
    if make_report:
        ...
        _report_ok = generate_report_pdf(
            ...
            report_data={
                ...
                'use_manual_sigma':  use_manual_sigma,
                'sigma1':            sigma1,
                'sigma2':            sigma2,
...
def save_settings(path=None):
    ...
    data = {
        ...
        'use_manual_sigma': var_use_manual_sigma.get(),
...
def load_settings(path=None):
    ...
    _setvar(var_use_manual_sigma, 'use_manual_sigma'); toggle_sigma_entries()
...
tk.Label(eng_fr, text=_pdal_hint, font=("Arial", 7),
         fg="#2E7D32" if _pdal_ok else "#C62828"
...
entry_sigma1 = _entry(pf, 8, "Sigma 1 [m]", "0.1"); entry_sigma1.config(state="disabled")
entry_sigma2 = _entry(pf, 9, "Sigma 2 [m]", "0.1"); entry_sigma2.config(state="disabled")
...
entry_clip_mask.grid(row=11, column=1, sticky="ew", **PAD)
tk.Label(pf, text="Mask file:").grid(row=11, column=0, sticky="e", **PAD)
...
btn_clip_browse.grid(row=11, column=2, sticky="w", **PAD)
tk.Label(pf, text="Clips all GeoTIFF outputs (rasters + DEM) to the first layer of the file.",
         fg="#666666", font=("Arial", 8)
         ).grid(row=12, column=1, columnspan=2, sticky="w", padx=6)

# ── EPSG code ──────────────────────────────────────────────────────
ttk.Separator(pf, orient="horizontal").grid(
    row=13, column=0, columnspan=3, sticky="ew", padx=4, pady=(6, 4))
tk.Label(pf, text="EPSG code:").grid(row=14, column=0, sticky="e", **PAD)
entry_epsg = tk.Entry(pf, width=10)
entry_epsg.grid(row=14, column=1, sticky="w", **PAD)
tk.Label(pf,
         ...
         ).grid(row=14, column=2, sticky="w", padx=4)
...
tk.Label(of, text="LAS extra fields:").grid(row=3, column=0, sticky="ne", **PAD)
las_fld_fr = tk.Frame(of); las_fld_fr.grid(row=3, column=1, columnspan=3, sticky="w", **PAD)
...
# ══════════════════════════════════════════════════════════════════════
# ── Volume Change Estimation ─────────────────────────────────────────
vcf = _lf(main_frame, "Volume Change Estimation  (M3C2 + DoD methods)", row=5)
...
# ── Wind Rose Export ────────────────────────────────────────────────
wrf = _lf(main_frame, "Wind Rose Export  (displacement direction & magnitude)", row=6)
...
# ── Profile Cut Export ───────────────────────────────────────────────
prf = _lf(main_frame, "Profile Cut Export  (interactive picker  or  TXT \u2192 PNG)", row=7)
...
ttk.Separator(main_frame, orient="horizontal").grid(
    row=8, column=0, columnspan=3, sticky="ew", padx=8, pady=(10, 4))

lbl_current_task = tk.Label(main_frame, text="Current Task: Ready", anchor="w", fg="#444444")
lbl_current_task.grid(row=9, column=0, columnspan=3, sticky="w", padx=10)
progress_current = ttk.Progressbar(main_frame, length=600, mode="determinate")
progress_current.grid(row=10, column=0, columnspan=3, padx=10, pady=(0,4), sticky="ew")

lbl_overall = tk.Label(main_frame, text="Overall Progress: Ready", anchor="w", fg="#444444")
lbl_overall.grid(row=11, column=0, columnspan=3, sticky="w", padx=10)
progress_overall = ttk.Progressbar(main_frame, length=600, mode="determinate")
progress_overall.grid(row=12, column=0, columnspan=3, padx=10, pady=(0,6), sticky="ew")
...
# ── Session save / load ──────────────────────────────────────────────
sess_fr = tk.Frame(main_frame)
sess_fr.grid(row=13, column=0, columnspan=3, pady=(4, 0))

tk.Button(sess_fr, text="\U0001f4be  Save session",
          command=save_settings,
          font=("Arial", 9), relief="flat",
          bg="#e3f2fd", fg="#1565c0", padx=14, pady=5,
          activebackground="#bbdefb", cursor="hand2"
          ).pack(side="left", padx=6)

tk.Button(sess_fr, text="\U0001f4c2  Load session",
...
tk.Label(sess_fr,
         text="Save / restore all settings as a .json file",
...
btn_start = tk.Button(main_frame, text="\u25ba   Run M3C2 Analysis",
                      ...
                      activebackground="#1b5e20", activeforeground="white")
btn_start.grid(row=14, column=0, columnspan=3, pady=(6, 16))
